...
public class Rectangle implements IntersectableRectangle {
	...
	public void draw(GraphicsHandler graphicsHandler) {
		...
		if (borderColor != null && !borderColor.equals(color)) {
			graphicsHandler.drawRectangle(Math.round(getX()), Math.round(getY()), getWidth(), getHeight(), borderColor,
					borderThickness);
		}
	}
	...
	public boolean intersects(IntersectableRectangle other) {
		...
		return Math.round(intersectRectangle.getX1()) < Math.round(otherIntersectRectangle.getX2() + 1)
				&& Math.round(intersectRectangle.getX2() + 1) > Math.round(otherIntersectRectangle.getX1()) &&
				Math.round(intersectRectangle.getY1()) < Math.round(otherIntersectRectangle.getY2() + 1)
				&& Math.round(intersectRectangle.getY2() + 1) > Math.round(otherIntersectRectangle.getY1());
	}

	// check if this is touching (side by side) or overlapping with another
	// rectangle
